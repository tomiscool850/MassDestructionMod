...
namespace MassDestruction.Items.projectiles.MageP
{
	...
	public class TheDirtWandProjectile : ModProjectile
	{
		...
		public override void AI()
		{
			...
			if (projectile.soundDelay == 0 && Math.Abs(projectile.velocity.X) + Math.Abs(projectile.velocity.Y) > 1f)
			{
			...
			if (Main.myPlayer == projectile.owner && projectile.ai[0] == 0f)
			{
				...
				if (player.channel)
				{
					float maxDistance = 50f; // This also sets the maximun speed the projectile can reach while following the cursor.
					...
					int velocityXBy1000 = (int)(vectorToCursor.X * 500f);
					int oldVelocityXBy1000 = (int)(projectile.velocity.X * 500f);
					int velocityYBy1000 = (int)(vectorToCursor.Y * 500f);
					int oldVelocityYBy1000 = (int)(projectile.velocity.Y * 500f);
				...
				else if (projectile.ai[0] == 0f)
				{
					...
					float maxDistance = 8f; // This also sets the maximun speed the projectile can reach after it stops following the cursor.
					Vector2 vectorToCursor = Main.MouseWorld - projectile.Center;
					float distanceToCursor = vectorToCursor.Length();
