...
{
	public class TitaniumJavilenProjectile : ModProjectile
	{
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Titanium Javilen");
		}
		...
		public override void Kill(int timeLeft)
		{
			...
			if (projectile.owner == Main.myPlayer)
			{
				...
				Main.rand.NextBool(18)
					? Item.NewItem(projectile.getRect(), ModContent.ItemType<TitaniumJavilen>())
					: 0;
		...
		public override void ModifyHitNPC(NPC target, ref int damage, ref float knockback, ref bool crit, ref int hitDirection)
		{
			...
			target.AddBuff(BuffID.Bleeding, 100); // Adds the ExampleJavelin debuff for a very small DoT

			projectile.damage = 10; // Makes sure the sticking javelins do not deal damage anymore
		...
		private void UpdateStickyJavelins(NPC target)
		{
			...
			for (int i = 0; i < Main.maxProjectiles; i++) // Loop all projectiles
			{
				...
				if (i != projectile.whoAmI // Make sure the looped projectile is not the current javelin
					...
					&& currentProjectile.modProjectile is TitaniumJavilenProjectile javelinProjectile // Use a pattern match cast so we can access the projectile like an ExampleJavelinProjectile
