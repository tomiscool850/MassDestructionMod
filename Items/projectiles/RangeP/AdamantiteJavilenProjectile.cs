...
{
	public class AdamantiteJavilenProjectile : ModProjectile
	{
		public override void SetStaticDefaults()
		{
			DisplayName.SetDefault("Adamantite Javilen");
		}
		...
		public override void Kill(int timeLeft)
		{
			...
			if (projectile.owner == Main.myPlayer)
			{
				...
				Main.rand.NextBool(18)
					? Item.NewItem(projectile.getRect(), ModContent.ItemType<AdamantiteJavilen>())
					: 0;
		...
		private void UpdateStickyJavelins(NPC target)
		{
			...
			for (int i = 0; i < Main.maxProjectiles; i++) // Loop all projectiles
			{
				...
				if (i != projectile.whoAmI // Make sure the looped projectile is not the current javelin
					...
					&& currentProjectile.modProjectile is AdamantiteJavilenProjectile javelinProjectile // Use a pattern match cast so we can access the projectile like an ExampleJavelinProjectile
